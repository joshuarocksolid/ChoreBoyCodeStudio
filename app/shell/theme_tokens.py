...
from app.shell.theme_toolbar_presets import TOOLBAR_PRESETS
...
def _shell_theme_tokens_dark(
    ...
    *,
    ...
    surfaces = DARK_CHROME_SURFACES.get(
        dark_chrome_palette,
        DARK_CHROME_SURFACES[constants.UI_THEME_DARK_CHROME_PALETTE_DEFAULT],
    )
    shared = {key: value for key, value in DARK_CHROME_SHARED.items() if key != "is_dark"}
    return ShellThemeTokens(
        **surfaces,
        **shared,
        is_dark=True,
        ui_font_weight_css=ui_font_weight_css,
        **TOOLBAR_PRESETS["dark"],
        **syntax_kwargs,
    )
