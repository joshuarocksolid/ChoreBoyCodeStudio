...
def install_main_window_composition(
    ...
    *,
    ...
    window._auto_save_to_file_timer.timeout.connect(window._save_workflow.flush_auto_save_to_file)
    window._pending_realtime_lint_file_path: str | None = None
    window._realtime_lint_timer = QTimer(window)
    window._realtime_lint_timer.setSingleShot(True)
    window._realtime_lint_timer.setInterval(300)
    ...
    window._local_history_workflow = LocalHistoryWorkflow(
        ...
        show_status_message=lambda message, timeout: window.statusBar().showMessage(message, timeout),
        logger=window._logger,
        background_tasks=window._background_tasks,
        retention_policy=window._local_history_retention_policy,
        ensure_breakpoint_spec=window._debug_control_workflow.ensure_breakpoint_spec,
        ...
        capture_tree_state=lambda: window._project_tree_presenter.capture_view_state(),
        restore_tree_state=lambda tree_state: window._project_tree_presenter.restore_view_state(tree_state),
        reveal_tree_path=lambda file_path: window._project_tree_presenter.reveal_path(file_path),
        set_tree_reveal_suppressed=lambda suppressed: window._project_tree_presenter.set_reveal_suppressed(
            suppressed
        ),
    )
    ...
    window._realtime_lint_timer.timeout.connect(
        build_realtime_lint_runner(
