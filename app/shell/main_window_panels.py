...
def _restore_python_console_history(window: Any) -> None:
    if window._python_console_widget is None:
        return
    entries = load_python_console_history(
        window._python_console_history_path,
        max_entries=200,
    )
    window._python_console_widget.set_history(entries)
...
def build_left_panel(window: Any) -> QWidget:
    ...
    window._activity_bar.view_changed.connect(workflow.handle_sidebar_view_changed)
    ...
    window._search_sidebar.preview_file_at_line.connect(workflow.handle_search_preview_file_at_line)
    window._search_sidebar.open_file_at_line.connect(workflow.handle_search_open_file_at_line)
...
def build_explorer_page(window: Any) -> QWidget:
    ...
    window._project_tree_widget.itemActivated.connect(workflow.handle_project_tree_item_activation)
    window._project_tree_widget.itemClicked.connect(workflow.handle_project_tree_item_click)
    window._project_tree_widget.setContextMenuPolicy(Qt.CustomContextMenu)
    window._project_tree_widget.customContextMenuRequested.connect(workflow.show_project_tree_context_menu)
    ...
    window._outline_panel.symbol_activated.connect(window._editor_tab_workflow.handle_outline_symbol_activated)
    window._outline_panel.collapsed_changed.connect(window._shell_layout_workflow.handle_outline_collapsed_changed)
    window._outline_panel.follow_cursor_changed.connect(window._shell_layout_workflow.handle_outline_follow_cursor_changed)
    window._outline_panel.sort_mode_changed.connect(window._shell_layout_workflow.handle_outline_sort_mode_changed)
    window._outline_panel.hide_requested.connect(window._shell_layout_workflow.handle_outline_hide_requested)
    ...
    window._shell_layout_workflow.apply_outline_layout_state()

    layout.addWidget(window._explorer_splitter, 1)
    workflow.update_explorer_buttons_enabled()
    return page
...
def build_center_panel(window: Any) -> QWidget:
    ...
    window._runtime_onboarding_workflow.connect_welcome_widget_actions(window._welcome_widget)
    ...
    tab_bar.set_tab_double_click_callback(window._editor_tab_workflow.handle_editor_tab_header_double_click)
    tab_bar.setContextMenuPolicy(Qt.CustomContextMenu)
    tab_bar.customContextMenuRequested.connect(window._editor_tab_workflow.show_editor_tab_context_menu)
    window._editor_tabs_widget.setTabBar(tab_bar)
    window._editor_tabs_widget.setObjectName("shell.editorTabs")
    window._editor_tabs_widget.currentChanged.connect(window._editor_tab_workflow.handle_editor_tab_changed)
    window._editor_tabs_widget.setTabsClosable(True)
    window._editor_tabs_widget.tabCloseRequested.connect(window._editor_tab_workflow.handle_tab_close_requested)
    ...
    window._runtime_onboarding_workflow.refresh_welcome_project_list()
...
def build_bottom_panel(window: Any) -> QWidget:
    ...
    window._problems_panel.item_preview_requested.connect(
        lambda file_path, line_number: window._editor_tab_workflow.open_file_at_line(
            file_path,
            line_number,
            preview=True,
        )
    )
    window._problems_panel.item_activated.connect(
        lambda file_path, line_number: window._editor_tab_workflow.open_file_at_line(
            file_path,
            line_number,
            preview=False,
