...
- `zoom` **View → Zoom In / Out / Reset** (Ctrl+=, Ctrl+-, Ctrl+0).
...
- **Chips after launch.** Read `#shell.startupStatusLabel` (ready), `#shell.runStatusLabel` (idle), `#shell.projectStatusLabel` (empty or name). Shot `status-launch`.
