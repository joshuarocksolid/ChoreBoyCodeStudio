...
- [Shop-LAN state](./shop-lan-state.md) — opt-in shared state root; product default is the probed trio owned by hidden-path-policy
- [Hidden path policy](./hidden-path-policy.md) — per-parent hidden file/dir probe matrix; probed product state default; first launch on the real default; Desktop icon sidecar fallback
