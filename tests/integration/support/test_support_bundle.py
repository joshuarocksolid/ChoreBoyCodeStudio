...
def test_build_support_bundle_includes_expected_artifacts(tmp_path: Path) -> None:
    """Support bundle zip should contain manifest, app log, run log, and diagnostics."""
    project_root = tmp_path / "project"
    state_root = tmp_path / "state"
    _write_valid_project(project_root)
    run_log_path = project_root / "logs" / "run_20260301_010101_aaaaaa.log"
    run_log_path.write_text("run output\n", encoding="utf-8")
    ...
    with zipfile.ZipFile(bundle_path, "r") as archive:
        ...
        assert f"project/logs/{run_log_path.name}" in names
