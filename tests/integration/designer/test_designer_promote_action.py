"""Integration test for designer promote-widget action."""
...
def test_promote_widget_action_updates_active_designer_surface(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _ensure_qapplication(monkeypatch)
    state_root = tmp_path / "state"
    state_root.mkdir(parents=True, exist_ok=True)
    project_root = tmp_path / "project"
    create_blank_project(str(project_root.resolve()), project_name="Designer Promote")
    ...
    responses = iter([("FancyButton", True), ("fancy_button", True)])
    monkeypatch.setattr(main_window_module.QInputDialog, "getText", lambda *args, **kwargs: next(responses))
    monkeypatch.setattr(main_window_module.QMessageBox, "warning", lambda *args, **kwargs: 0)
    ...
    window._handle_designer_promote_widget_action()

    promoted = surface.model.root_widget.find_by_object_name("pushButton")  # type: ignore[union-attr]
    assert promoted is not None
    assert promoted.class_name == "FancyButton"
    assert len(surface.model.custom_widgets) == 1  # type: ignore[union-attr]
    window.close()
