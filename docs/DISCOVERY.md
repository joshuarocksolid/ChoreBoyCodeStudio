...
## 3B. Desktop Installer Launch Standard: Path-Keyed Package Root (2026-05-30)

The `desktop_installer_launch_probe` confirmed the installer launcher standard:

```text
STANDARD_CONFIRMED: path_key_env_root_desktop
```

Installable package launchers should use a `Path=<absolute package root>` key and
a direct AppRun command. The inline Python must capture `os.getcwd()` into
`CBCS_PACKAGE_ROOT` before running `installer/bootstrap.py`.
...
StartupNotify=true
Path=/home/default/my_app_installer
Exec=/opt/freecad/AppRun -c "import os,runpy,sys;root=os.getcwd();os.environ['CBCS_PACKAGE_ROOT']=root;entry=os.path.join(root,'installer','bootstrap.py');sys.path.insert(0,os.path.dirname(entry)) if os.path.dirname(entry) not in sys.path else None;runpy.run_path(entry, run_name='__main__')"
```

Do not use `/bin/sh -c ... %k`, shell wrapper scripts, direct AppRun `%k`
inspection, or `Terminal=true` diagnostics for installer launchers. Those variants
failed silently or invoked missing terminal tools during real ChoreBoy desktop
double-click tests.

This is Path-keyed relocation, not true self-location. If the installer package
folder moves after export, rewrite the `.desktop` `Path=` value to the new
absolute package root before launching. Installed app launchers remain fixed-root
to the chosen install directory and should be regenerated by rerunning the
installer if that folder moves.
